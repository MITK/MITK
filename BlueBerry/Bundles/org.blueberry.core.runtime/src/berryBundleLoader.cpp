...
BundleLoader::GetContextForBundle(IBundle::ConstPointer bundle)
{
  ...
  return m_BundleMap[QString::fromStdString(bundle->GetSymbolicName())].m_Context;
}
...
BundleLoader::FindBundle(const std::string& symbolicName)
{
  ...
  iter = m_BundleMap.find(QString::fromStdString(symbolicName));
  if (iter == m_BundleMap.end()) return IBundle::Pointer();
  return iter.value().m_Bundle;
}
...
std::vector<IBundle::Pointer> BundleLoader::GetBundles() const
{
  ...
  {
    result.push_back(it.value().m_Bundle);
  }
  return result;
}
...
BundleLoader::LoadBundle(Bundle::Pointer bundle)
{
  ...
  {
    Poco::Mutex::ScopedLock lock(m_Mutex);
    if (m_BundleMap[QString::fromStdString(bundle->GetSymbolicName())].m_Bundle.IsNull())
    {
      BundleInfo bundleInfo;
      bundleInfo.m_Bundle = bundle;
      bundleInfo.m_ClassLoader = new ActivatorClassLoader();
      QDir path; Platform::GetStatePath(path, bundle);
      bundleInfo.m_Context = new BundleContext(*this, bundle, Poco::Path(path.absolutePath().toStdString()));
      m_BundleMap[QString::fromStdString(bundle->GetSymbolicName())] = bundleInfo;
...
BundleLoader::ResolveBundle(IBundle::Pointer bundle)
{
  ...
  catch (const BundleResolveException& exc)
  {
    BERRY_ERROR << "Bundle resolve failed: " << exc.what();
  }
...
BundleLoader::ResolveAllBundles()
{
  ...
  {
    this->ResolveBundle(iter.value().m_Bundle);
  }
}
...
void BundleLoader::ReadAllContributions()
{
  ...
  for (iter = m_BundleMap.begin(); iter != m_BundleMap.end(); ++iter)
  {
    if (iter.value().m_Bundle && iter.value().m_Bundle->IsResolved())
      this->ReadContributions(iter.value().m_Bundle);
  }
}
...
void BundleLoader::ReadDependentContributions(IBundle::Pointer bundle)
{
  ...
  {
    BundleMap::const_iterator depIter = m_BundleMap.find(QString::fromStdString(iter->symbolicName));
    if (depIter != m_BundleMap.end())
    {
      // only read contributions from legacy BlueBerry bundles
      if (IBundle::Pointer depBundle = depIter.value().m_Bundle)
      {
...
BundleLoader::StartAllBundles()
{
  ...
  for (iter = m_BundleMap.begin(); iter != m_BundleMap.end(); ++iter)
  {
    try
    {
      if  (iter.value().m_Bundle && iter.value().m_Bundle->GetActivationPolicy() == IBundleManifest::EAGER  &&
        !iter.value().m_Bundle->IsSystemBundle())
        this->StartBundle(iter.value().m_Bundle);
    }
...
BundleLoader::StartBundle(Bundle::Pointer bundle)
{
  ...
  BundleInfo info = m_BundleMap[QString::fromStdString(bundle->GetSymbolicName())];
...
BundleLoader::StartDependencies(Bundle::Pointer bundle)
{
  ...
  for (iter = deps.begin(); iter != deps.end(); ++iter)
  {
    ...
    if (depIter != m_BundleMap.end())
    {
      if (Bundle::Pointer depBundle = depIter.value().m_Bundle)
      {
