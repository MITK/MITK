...
template<class C>
C* BundleLoader::LoadClass(const QString& bundleName, const QString& className)
{
  return LoadClass<C>(bundleName, className, QString::fromStdString(C::GetManifestName()));
}
...
{
  BERRY_INFO(m_ConsoleLog) << "Trying to load class " << className.toStdString()
                           << " with manifest name " << manifestName.toStdString()
                           << " from bundle " << bundleName.toStdString();
  ...
  BundleInfo& bundleInfo = bundleIter.value();
  ...
  if (!cl->isLibraryLoaded(libPath.toString()))
  {
    ...
    try {
      cl->loadLibrary(libPath.toString(), manifestName.toStdString());
    }
  ...
  return cl->create(className.toStdString());
}
