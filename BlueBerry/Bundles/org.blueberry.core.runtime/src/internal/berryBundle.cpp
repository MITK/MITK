  ...
  if (m_State == BUNDLE_INSTALLED)
  {
    ...
    for (iter =  this->GetRequiredBundles().begin(); iter !=  this->GetRequiredBundles().end(); ++iter)
    {
      ...
      if (bundle.IsNull())
      {
        ...
        if (!resolved)
        {
          throw BundleResolveException(QString("The bundle ") + QString::fromStdString(this->GetSymbolicName()) + " depends on missing bundle: " + QString::fromStdString(iter->symbolicName));
        }
      }
...
//    BERRY_INFO << "Bundle " << this->GetSymbolicName() << " is active";
  ...
  {
    throw BundleStateException(QString("Bundle ") + QString::fromStdString(this->GetSymbolicName()) +
