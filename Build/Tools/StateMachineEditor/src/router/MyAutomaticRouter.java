 ...
 * possible.
 */
public abstract class MyAutomaticRouter extends MyAbstractRouter {
	
	private ConnectionRouter nextRouter;
	private MyMultiValueMap connections = new MyMultiValueMap();
	 ...
	 * @param index The index of the current connection in the list of colliding connections
	 */
	protected abstract void handleCollision1(PointList list, int index);
	...
	public void invalidate(Connection conn) {
		...
		if (affected != -1) {
			for (int i = 0/*affected*/; i < connectionList.size(); i++) {
	...
	public void route(Connection conn) {
		if (conn.getSourceAnchor().equals(conn.getTargetAnchor())) {
			if (next() != null) 
				next().route(conn);
			else {
			...
			if (conn.getPoints().size() == 2) {
				...
				if (connectionList != null) {
					sourceAnchor = ((Connection)connectionList.get(0)).getSourceAnchor();
					int index;
					...
					index2 = 0;
					if (sourcePoint == null) {
						sourcePoint = points.getFirstPoint();
					}
					if (connectionList.size() > 0) {
						connectionList = connections.get(connectionKey);
						for (int i = 0; i < connectionList.size(); i++) {
							ConnectionAnchor anchor3 = ((Connection)connectionList.get(i)).getSourceAnchor();
							if (anchor3 == sourceAnchor) {
								index1++;
								index = index1;
							}
							else {
					...
					handleCollision1(points, index);
					conn.setPoints(points);
				}
				else {
					sourceAnchor = conn.getSourceAnchor();
					handleCollision1(points, 1);
					conn.setPoints(points);
					connections.put(connectionKey, conn);
				}
			}	
