# Special version of BundleUtilities that calls to a local GetPrerequisites which implements a caching mechanism.
# Thank you to Daniel Maleike for the contribution

message("Using MITK version of BundleUtilities.cmake")
    ...
    if(IS_DIRECTORY "${app}")
      if(app MATCHES "\\.app$")
        ...
        if(EXISTS "${app}" AND EXISTS "${executable}")
          ...
          set(valid 1)
          #message("info: handled .app directory case...")
        else()
          message("warning: *NOT* handled - .app directory case...")
        endif()
      else()
        message("warning: *NOT* handled - directory but not .app case...")
      endif()
    else()
      ...
      if(is_executable)
        ...
        if(EXISTS "${dotapp_dir}")
          ...
          set(valid 1)
          #message("info: handled executable file in .app dir case...")
        else()
          get_filename_component(app_dir "${app}" PATH)
          set(${bundle_var} "${app_dir}" PARENT_SCOPE)
          set(${executable_var} "${app}" PARENT_SCOPE)
          set(valid 1)
          #message("info: handled executable file in any dir case...")
        endif()
      else()
        message("warning: *NOT* handled - not .app dir, not executable file...")
      endif()
    endif()
  else()
    message("warning: *NOT* handled - directory/file does not exist...")
  endif()
  ...
  if(valid)
    ...
    foreach(lib ${libs})
      ...
      if(NOT ignoreFile)
        ...
        message("Ignoring file: ${prereq_filename}")
      endif()
    endforeach()
...
function(link_resolved_item_into_bundle resolved_item resolved_embedded_item)
  ...
  if("${resolved_item_compare}" STREQUAL "${resolved_embedded_item_compare}")
    message("warning: resolved_item == resolved_embedded_item - not linking...")
  else()
...
function(copy_resolved_item_into_bundle resolved_item resolved_embedded_item)
  ...
  if("${resolved_item_compare}" STREQUAL "${resolved_embedded_item_compare}")
    message("warning: resolved_item == resolved_embedded_item - not copying...")
  else()
...
function(copy_resolved_framework_into_bundle resolved_item resolved_embedded_item)
  ...
  if("${resolved_item_compare}" STREQUAL "${resolved_embedded_item_compare}")
    ...
    if(BU_COPY_FULL_FRAMEWORK_CONTENTS)
      ...
      #message("copying COMMAND ${CMAKE_COMMAND} -E copy_directory '${resolved_dir}' '${resolved_embedded_dir}'")
      ...
      if(EXISTS "${resolved_resources}")
        #message("copying COMMAND ${CMAKE_COMMAND} -E copy_directory '${resolved_resources}' '${resolved_embedded_resources}'")
      ...
      if(NOT EXISTS "${resolved_resources}/Info.plist")
        ...
        if(EXISTS "${resolved_info_plist}")
          #message("copying COMMAND ${CMAKE_COMMAND} -E copy_directory '${resolved_info_plist}' '${resolved_embedded_info_plist}'")
...
function(fixup_bundle app libs dirs)
  #get_prerequisites_clear_cache()
  message("MITK fixup_bundle")
  message("fixup_bundle")
  message("  app='${app}'")
  message("  libs='${libs}'")
  message("  dirs='${dirs}'")
  #get_property(current_cache GLOBAL PROPERTY prerequisites_cachevariables)
  ...
  message("  ignoreItems='${CFG_IGNORE_ITEM}'")

  get_bundle_and_executable("${app}" bundle executable valid)
  if(valid)
    get_filename_component(exepath "${executable}" PATH)

    message("fixup_bundle: preparing...")
    get_bundle_keys("${app}" "${libs}" "${dirs}" keys IGNORE_ITEM "${CFG_IGNORE_ITEM}")

    message("fixup_bundle: copying...")
    list(LENGTH keys n)
    math(EXPR n ${n}*2)
    ...
    foreach(key ${keys})
      ...
      if("${${key}_COPYFLAG}" STREQUAL "2")
        message("${i}/${n}: linking '${${key}_RESOLVED_ITEM}' -> '${${key}_RESOLVED_EMBEDDED_ITEM}'")
      elseif(${${key}_COPYFLAG})
        message("${i}/${n}: copying '${${key}_RESOLVED_ITEM}'")
      else()
        message("${i}/${n}: *NOT* copying '${${key}_RESOLVED_ITEM}'")
      endif()

      set(show_status 0)
      if(show_status)
        message("key='${key}'")
        message("item='${${key}_ITEM}'")
        message("resolved_item='${${key}_RESOLVED_ITEM}'")
        message("default_embedded_path='${${key}_DEFAULT_EMBEDDED_PATH}'")
        message("embedded_item='${${key}_EMBEDDED_ITEM}'")
        message("resolved_embedded_item='${${key}_RESOLVED_EMBEDDED_ITEM}'")
        message("copyflag='${${key}_COPYFLAG}'")
        message("")
      endif()
    ...
    message("fixup_bundle: fixing...")
    foreach(key ${keys})
      math(EXPR i ${i}+1)
      if(APPLE)
        message("${i}/${n}: fixing up '${${key}_RESOLVED_EMBEDDED_ITEM}'")
        if(NOT "${${key}_COPYFLAG}" STREQUAL "2")
          fixup_bundle_item("${${key}_RESOLVED_EMBEDDED_ITEM}" "${exepath}" "${dirs}")
        endif()
      else()
        message("${i}/${n}: fix-up not required on this platform '${${key}_RESOLVED_EMBEDDED_ITEM}'")
      endif()
    endforeach()
    ...
    #TODO:
    message("DON'T VERIFY_APP, WE KNOW OUR CPACK IS BROKEN")
...
function(verify_bundle_prerequisites bundle result_var info_var)
  ...
  foreach(f ${file_list})
      ...
      message("executable file ${count}: ${f}")
