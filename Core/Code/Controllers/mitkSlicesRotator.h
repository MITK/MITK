...
/**
  ...
  "normal" behaviour of SliceNavigationControllers (which is picking one plane from a stack of planes). 
  
  This additional class SlicesRotator is needed, because one has to be aware of multiple "visible slices" 
  (selected Geometry2Ds of some SliceNavigationControllers) in order to
  choose between rotation and slice selection. Such functionality could not be implemented by a single SliceNavigationController.
 
  Rotation is achieved by modifying (rotating) the generated
  TimeSlicedGeometry of the corresponding SliceNavigationControllers.
  ...
  planes, which are "near" the cursor. If this number is 2 (one for the 
  ...
  the cursor. If the "LinkPlanes" flag is set, the rotation is applied to the
  ...
  exactly three 2D vies in our scene. This used to be the standard setup of the MITK associated application for a long time.
  With custom applications based on MITK it is easy to create different situations. 
  One usual use case would be to have one extra render window display the contents of any
  of the other ones and behave exactly like it (could e.g. be used on a second screen).

  In this situation the above assumption "we rotate when there are exactly 2 slices close to the cursor"
  will not hold: since we always have two render windows displaying the exact same slice,
  the number of 2 is the minimum we get. Whenever the user clicks in one of those windows
  and the cursor is close to one of the orthogonal planes, we will get a count of 3 or more planes that are
  "close to the cursor".
  ...
  For the class to behave correctly, we actually need to distinguish three separate cases:
   ...
   The "counting nearby lines in the renderwindow" can also work for the general case described above.
   Only one details needs to be accounted for: we must not count a line when it is identical to another
   line. I.e. we just count how many visible lines on the screen are very close to the cursor. When this
   number is 1, we rotate, otherwise we let the SliceNavigationControllers do their slice selection job.
...
public:
  ...
  */
  ...
  /**
    \brief NOT USED by anything open-source. There is one DKFZ-internal call ("Mytralizer").

    \TODO check if this is actually still needed. 
    With SliceNavigationController::ReorientSlices() now being implemented this could be obsolete!?
  */
...
protected:
  ...
  /// Called from SlicesCoordinator after a new controller is added (to internal list m_SliceNavigationControllers).
  virtual void OnSliceControllerAdded(SliceNavigationController* snc);

  /// Called from SlicesCoordinator after a new controller is being removed (to internal list m_SliceNavigationControllers).
  ...
  */
  ...
  // TODO following methods called from superclass ExecuteAction
