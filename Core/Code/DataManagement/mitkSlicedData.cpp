...
void mitk::SlicedData::Graft( const itk::DataObject *data )
{
  ...
  if ( data )
  {
    // Attempt to cast data to a SlicedData object
    const Self *slicedData;

    try
    {
      slicedData = dynamic_cast< const Self * >( data );
    }
    catch( ... )
    {
      return;
    ...
    if ( slicedData )
    {
      ...
      // Copy the meta-information (possibly the sub-class implementation of
      // CopyInformation() is called, which should in return call its superclass
      // implementation
      this->CopyInformation( slicedData );

      // Copy the remaining region information. Subclasses are
      // responsible for copying the pixel container.
      this->SetRequestedRegion( const_cast< Self * >( slicedData ) );
    }
    else
    {
      // pointer could not be cast back down
