...
void mitk::FileWriterManager::Write(mitk::BaseData::Pointer data, const std::string& path, us::ModuleContext* /*context*/)
{
  // Find extension
...
std::vector <mitk::IFileWriter*> mitk::FileWriterManager::GetWriters(const std::string& extension, us::ModuleContext* context )
{
  ...
  std::vector <us::ServiceReference<IFileWriter> > refs = GetWriterList(extension, context);

  // Translate List of ServiceRefs to List of Pointers
  for (std::vector <us::ServiceReference<IFileWriter> >::const_iterator iter = refs.begin();
       iter != refs.end(); ++iter)
  {
...
{
  std::vector <mitk::IFileWriter*> matching = mitk::FileWriterManager::GetWriters(extension, options, context);
...
{
  std::vector <mitk::IFileWriter*> allWriters = mitk::FileWriterManager::GetWriters(extension, context);
  std::vector <mitk::IFileWriter*> result;
  // the list is always sorted by priority. Now find Writer that supports all options

  for (std::vector <IFileWriter*>::const_iterator iter = allWriters.begin();
...
{
  us::ModuleContext* context = us::GetModuleContext();
  std::vector<us::ServiceReference<IFileWriter> > refs = GetWriterListByBasedataType(basedataType, context);
  return CreateFileDialogString(refs);
}
...
std::string mitk::FileWriterManager::CreateFileDialogString(const std::vector<us::ServiceReference<IFileWriter> >& refs)
{
  ...
  for (std::vector<us::ServiceReference<IFileWriter> >::const_iterator iterator = refs.begin(), end = refs.end(); iterator != end; ++iterator)
  {
...
{
  // filter for class and extension
  std::string filter = "(" + mitk::IFileWriter::PROP_BASEDATA_TYPE + "=" + basedataType + ")";
  std::vector <us::ServiceReference<IFileWriter> > result = context->GetServiceReferences<IFileWriter>(filter);
  std::sort(result.begin(), result.end());
  std::reverse(result.begin(), result.end());
  return result;
