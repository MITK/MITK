...
void mitk::PointSetInteractor::UnselectAll()
{
  ...
  for (it = itkPointSet->GetPoints()->Begin(); it != end; it++)
  {
    ...
    if ( pointData && pointData.selected )//then declare an operation which unselects this point; UndoOperation as well!
    {
...
bool mitk::PointSetInteractor::ExecuteAction( Action* action, mitk::StateEvent const* stateEvent )
{
  ...
  case AcREMOVEPOINT://remove the given Point from the list
    ...
    // Remove all Points that have been set at once. TODO: Undo function not supported yet.
  case AcREMOVEALL:
    {
      if ( !points->empty() )
      {
        ...
        while ( it != end )
        {
          ...
          {
            pt = pointSet->GetPoint( position );
            PointOperation* doOp = new mitk::PointOperation( OpREMOVE, pt, position );
            ++it;
            pointSet->ExecuteOperation( doOp );
          }
          else it++;
      ...
      while( it != end )
      {
        ...
        if ( pointSet->GetSelectInfo(position) )//if selected
        {
          ...
          if ( m_UndoEnabled )//&& (posEvent->GetType() == mitk::Type_MouseButtonRelease)
          {
            ...
            OperationEvent *operationEvent = new OperationEvent(pointSet, doOp, undoOp, "Move point");
            m_UndoController->SetOperationEvent(operationEvent);
          }
          //execute the Operation
          pointSet->ExecuteOperation(doOp);
