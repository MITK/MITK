...
namespace mitk
{
  ...
  const mitk::PixelType BoundingShapeCropper::GetOutputPixelType()
  {
  ...
  void BoundingShapeCropper::GenerateInputRequestedRegion()
  {
    ...
    if ((output->IsInitialized() == false) || (m_Geometry.IsNull()) || (m_Geometry->GetTimeGeometry()->CountTimeSteps() == 0))
      return;
  ...
  void BoundingShapeCropper::GenerateOutputInformation()
  {
    ...
    mitk::BoundingBox::Pointer bsBoxRelativeToImage = bsGeometry->CalculateBoundingBoxRelativeToTransform(inputImageGeometry->GetIndexToWorldTransform());
