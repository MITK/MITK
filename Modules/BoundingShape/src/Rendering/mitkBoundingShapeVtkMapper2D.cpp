...
bool mitk::BoundingShapeVtkMapper2D::LocalStorage::IsUpdateRequired(
...
void mitk::BoundingShapeVtkMapper2D::GenerateDataForRenderer(BaseRenderer* renderer)
{
  ...
  bool needGenerateData = localStorage->IsUpdateRequired(renderer,this,GetDataNode());// true; // localStorage->GetLastGenerateDataTime() < node->GetMTime() || localStorage->GetLastGenerateDataTime() < node->GetData()->GetMTime(); //localStorage->IsGenerateDataRequired(renderer, this, GetDataNode());

  double scale = renderer->GetScaleFactorMMPerDisplayUnit();

  if (abs(scale - localStorage->m_ZoomFactor) > 0.001)
  {
