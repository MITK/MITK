...
std::string
mitk::GeneratePropertyNameForDICOMTag(unsigned int group, unsigned int element)
{
    std::ostringstream nameStream;
    nameStream << "DICOM." << std::setw(4) << std::setfill('0') << std::hex << group << "." << std::setw(4) << std::setfill('0') << std::hex << element;

    return nameStream.str();
};

bool
mitk::GetBackwardsCompatibleDICOMProperty(unsigned int group, unsigned int element,
