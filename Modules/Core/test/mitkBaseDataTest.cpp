...
public:
  ...
  void GetOriginOfCloneBaseData()
  {
	...
	m_CloneBaseData = m_BaseDataImpl->Clone();
	CPPUNIT_ASSERT_MESSAGE("Testing origin set in clone!", m_CloneBaseData->GetGeometry(1)->GetOrigin() == m_Geo3->GetOrigin());
  }
  ...
  void BasePropertyListIsSet_Success()
  {
	...
	CPPUNIT_ASSERT_MESSAGE("Check if base property list is set correctly!", m_BaseDataImpl->GetPropertyList() == m_PropertyList);
  }
  ...
  void CopyingInformationOfBaseData_Failure() 
  {
	...
	CPPUNIT_ASSERT_MESSAGE("Check copying of Basedata Data Object!", newBaseData->GetTimeGeometry()->CountTimeSteps() != 5);
  }
};
