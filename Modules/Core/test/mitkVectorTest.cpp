...
public:
	...
	void ItkVecorEqualityUsingDifferentVectors_Failure()
	{
		...
		CPPUNIT_ASSERT_NO_THROW_MESSAGE("Test vector equality using different vectors with an element-wise difference greater than mitk::eps",
	...
	void ItkPointEqualitySamePoint_Success()
	{
		...
		CPPUNIT_ASSERT_MESSAGE("Test point equality using the same point with mitk::eps", mitk::Equal(m_ItkPoint_1, m_ItkPoint_1));
	}
	...
	void ItkPointEqualityDifferentPoints_Failure()
	{
		...
		CPPUNIT_ASSERT_NO_THROW_MESSAGE("Test point equality using different points with an element-wise difference greater than mitk::eps",
    ...
    void ItkPointEqualitForHigherEpsilons_Success()
	{
		...
		CPPUNIT_ASSERT_MESSAGE("Points are equal for higher epsilon tolerance ( 1.2 * mitk::eps )", mitk::Equal(m_ItkPoint_1, m_ItkPoint_2, mitk::eps * 1.2));
	}
	...
	void MitkVnlVectorEqualitySameMitkVnlVector_Success() 
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			mitkVnlVector_3.put(i, m_ItkVector_1[i]);
		}
	...
	void MitkVnlVectorEqualityDifferentMitkVnlVectors_Failure()
	{
		...
		CPPUNIT_ASSERT_NO_THROW_MESSAGE("Test mitk vnl vector equality using different mitk vnl vectors with an element-wise difference " "greater than mitk::eps",
	...
	void VnlVectorEqualitySameVnlVector_Success()
	{
		...
		for (int i = 0; i < 7; i++)
		{
			if (i < 3)
			{
				m_VnlVector_1.put(i, m_ItkVector_1[i]);
			}
	...
	void VnlVectorEqualityDifferentVnlVectors_Failure()
	{
		...
		CPPUNIT_ASSERT_NO_THROW_MESSAGE("vnl_fixed : v_1 != v_2 with mitk::eps ", (mitk::Equal<VnlValueType, 7>(m_VnlVector_1, m_VnlVector_2)));
	}
	...
	void VnlVectorEqualityDifferentVnlVectorsWithHighEps_Success()
	{
		...
		CPPUNIT_ASSERT_MESSAGE("vnl_fixed : v_1 == v_2 with eps = 1.2 * mitk::eps ", (mitk::Equal<VnlValueType, 7>(m_VnlVector_1, m_VnlVector_2, mitk::eps * 1.2f)));
	}
	...
	{
		CPPUNIT_ASSERT_NO_THROW_MESSAGE("Test scalar equality using the different scalars with a difference greater than mitk::eps", 
	...
	{
		CPPUNIT_ASSERT_NO_THROW_MESSAGE("Test for matrix equality with given epsilon=0.0 and slightly different matrix elements",
