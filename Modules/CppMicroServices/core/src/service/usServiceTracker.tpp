...
void ServiceTracker<S,TTT>::Open()
{
  _TrackedService* t;
  {
    typename _ServiceTrackerPrivate::Lock lock(d);
    if (d->trackedService)
    {
      return;
    }
    ...
    {
      typename _TrackedService::Lock lock2(*t);
      try {
...
void ServiceTracker<S,TTT>::Close()
{
  ...
  {
    typename _TrackedService::Lock lock(outgoing);
...
ServiceTracker<S,TTT>::WaitForService(unsigned long timeoutMillis)
{
  ...
  while (!TTT::IsValid(object))
  {
    ...
    {
      typename _TrackedService::Lock lock(t);
      if (t->Size() == 0)
      {
