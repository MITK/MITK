...
class MITKDICOM_EXPORT DICOMDatasetSorter : public itk::LightObject
{
  public:
    ...
    */
    ...
    const DICOMSplitReason* GetSplitReason(unsigned int index) const;
  ...
  protected:
    ...
    void SetOutput(unsigned int index, const DICOMDatasetList& output, const DICOMSplitReason* splitReason = nullptr);

  private:

    DICOMDatasetList m_Input;
    std::vector< DICOMDatasetList > m_Outputs;
    std::vector< DICOMSplitReason::Pointer > m_SplitReasons;
};
