...
//#include <..\ITKV3Compatibility\itkInverseDeformationFieldImageFilter.h>
    ...
    void SymmetricForcesDemonsRegistration::GenerateData2( itk::Image<TPixel, VImageDimension>* itkImage1)
  {
    ...
    if (fixedImage.IsNotNull() && movingImage.IsNotNull())
    {
      ...
      else
      {
        ...
        //m_DeformationField = filter->GetOutput(0);
        m_DeformationField = (itk::Image<itk::Vector<float, 3>,3> *)(filter->GetOutput(0)); //see BUG #3732
        if(m_SaveField)
        {
          fieldwriter->Update();
        }
