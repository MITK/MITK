...
const float* test(caffe::shared_ptr<caffe::Net<float>> network)
{
  int num_features = 100;
  float *data = new float[1*1*num_features];
  float *label = new float[1*1*1];

  caffe::MemoryDataLayer<float> *image_input_layer = (caffe::MemoryDataLayer<float> *) (network->layer_by_name("image_data").get());
  image_input_layer->Reset(data, label, 1);
  network->Forward();
  boost::shared_ptr<caffe::Blob<float> > output_layer = network->blob_by_name("direction");
...
//  // only for fully connected
//  input_layer->Reshape(1, num_channel, height, width);
...
/*!
* \brief Minimal example for Caffe usage in MITK
*
*/
...
//  const float* test_output = test(trained_net);
  ...
  network.reset(new caffe::Net<float>(network_file, caffe::TEST));
  network->CopyTrainedLayersFrom(weights_file);

  MITK_INFO << "predicting";
  std::vector<float> output = predict_data_layer(img, network);
