...
#define QBALL_ANAL_RECON_PI       M_PI
  ...
  template<
    ...
    class TOdfPixelType,
      int NOrderL,
      int NrOdfDirections>
      typename itk::DiffusionMultiShellQballReconstructionImageFilter<
      TReferenceImagePixelType,TGradientImagePixelType,TOdfPixelType,
      NOrderL,NrOdfDirections>::OdfPixelType
      itk::DiffusionMultiShellQballReconstructionImageFilter
      <TReferenceImagePixelType, TGradientImagePixelType, TOdfPixelType,
    ...
    template<
        ...
        case QBAR_B_ZERO_B_VALUE:
          {
            ...
            for(int i=0; i<n; i++)
            {
              if (vec[i]<=0)
              {
                MITK_ERROR << "DiffusionMultiShellQballReconstructionImageFilter: negative dwi image value";
                vec[i] = 0.001;
              }
              vec[i] = log(vec[i]);
            }
      ...
      template< class T, class TG, class TO, int L, int NODF>
      double DiffusionMultiShellQballReconstructionImageFilter<T,TG,TO,L,NODF>
