...
void TestGoToSnapshotException()
{
 //testing GoToSnapShot for exception
  ...
  //std::string file2 = mitk::StandardFileLocations::GetInstance()->FindFile("NavigationDataTestDataInvalidTags.xml", "Modules/IGT/Testing/Data");
  ...
  try
  {
    myTestPlayer2->GoToSnapshot(1000000);
  }
  catch(mitk::IGTException)
  {
   exceptionThrown2=true;
   MITK_TEST_OUTPUT(<<"Tested exception for the case when cannot go back to particular snapshot in GoToSnapShot. Application should not crash.");
  }
  MITK_TEST_CONDITION_REQUIRED(exceptionThrown2, "Testing  GoToSnapShot method if exception (cannot go back to particular snapshot) was thrown.");
...
const char* XML_TESTSTRING =
  ...
  //The string above XML_TESTSTRING is a wrong string, some element were deleted in above
  try
  {
   myTestPlayer3->SetXMLString(XML_TESTSTRING);
  } 
  catch(mitk::IGTException)
  {
  exceptionThrown3=true;
  MITK_TEST_OUTPUT(<<"Tested exception for the case when. Application should not crash.");
  }
 MITK_TEST_CONDITION_REQUIRED(exceptionThrown3, "Testing SetXMLString method if exception (string is not an XML String) was thrown.");
}
