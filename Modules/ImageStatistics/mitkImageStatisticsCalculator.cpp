...
namespace mitk
{
    ...
    itk::Image<unsigned short, VImageDimension> *maskImage)
  {
    ...
    minMax.MaxIndex.set_size(inputImage->GetImageDimension());
     ...
     Converting IndexType in vnl_vector<int>
  ...
  {
    typedef itk::Image< TPixel, VImageDimension > ImageType;
    typedef itk::Image<float, VImageDimension> ConvolutionMaskImageType;
    ...
    typedef typename ConvolutionMaskImageType::IndexType IndexType;
    IndexType start;
    start.Fill(0);

    typedef typename ConvolutionMaskImageType::SizeType SizeType;
    SizeType size;
    ...
    typedef typename ConvolutionMaskImageType::RegionType RegionType;
    RegionType region;
    region.SetSize(size);
    region.SetIndex(start);

    ConvolutionMaskImageType::Pointer convolutionMask = ConvolutionMaskImageType::New();
    ...
    typedef typename ConvolutionMaskImageType::PointType PointType;
    typedef itk::ImageRegionConstIteratorWithIndex<ConvolutionMaskImageType> ConvolutionMaskIteratorType;
    ConvolutionMaskIteratorType maskIt(convolutionMask,region);
    ...
    typedef itk::FFTConvolutionImageFilter<ImageType, ConvolutionMaskImageType, PeakImageType> FilterType;
    ...
    convolutionFilter->SetKernelImage(convolutionMask.GetPointer());
    ...
    float hotspotPeak = itk::NumericTraits<float>::min();
    ...
    int offsetX = static_cast<int>((radiusInMM / maskSpacing[0]) + 0.9999 );
    int offsetY = static_cast<int>((radiusInMM / maskSpacing[1]) + 0.9999 );
    int offsetZ = static_cast<int>((radiusInMM / maskSpacing[2]) + 0.9999 );
    ...
    CroppedImageIteratorType sphereMaskIt(croppedRegionMask, croppedRegionMask->GetLargestPossibleRegion());
     ...
     Creating mask which gurantees that sphere is inside the given region in any case
    for(sphereMaskIt.GoToBegin(); !sphereMaskIt.IsAtEnd(); ++sphereMaskIt)
    {
      SphereMaskImageType::IndexType index = sphereMaskIt.GetIndex();
    ...
    for(int i = 0; i < VImageDimension; ++i)
      translation[i] = ((hotspotSphereSize[i] ) / 2) * hotspotSphereSpacing[i];
    ...
    croppedOutputRegion.SetSize(croppedSize);
    ...
    ImageIteratorType inputIt(inputImage, croppedRegion);
