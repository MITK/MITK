...
, m_ThreadID(-1)
{
  m_Command = ReceptorCommand::New(); //not nice, because each object sets a new static command
...
{
  // it is possible to execute this code directly in the loop for 
  // the thread, but the delay is far greater
  if(IR_CHANGED && changed && m_WiiMote.IR.Dot[0].bVisible)
  {
...
void mitk::WiiMoteThread::StartWiiMote()
{
  ...
  while(!m_WiiMote.Button.Home() && !m_StopWiiMote )
  {
    m_WiiMoteThreadFinished->Lock();
