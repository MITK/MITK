...
{
   const map::events::AnyMatchPointEvent* pMAPEvent =
       dynamic_cast<const map::events::AnyMatchPointEvent*>(&event);
...
run()
{
   ...
   if (m_doGeometryRefinement)
   {
      ...
      {
         emit Error(QString("Error when refining image geometry. Error description: ") + QString::fromLatin1(
                        e.what()));
      }
      catch (...)
      {
   ...
   else
   {
      try
      {
         ...
         if (inputImage)
         {
            spResultData = mitk::ImageMappingHelper::map(this->GetInputDataAsImage(), this->GetRegistration(),
                           !(this->m_allowUndefPixels), this->m_paddingValue, this->m_spRefGeometry,
                           !(this->m_allowUnregPixels), this->m_errorValue, this->m_InterpolatorType).GetPointer();
         }
         else if (inputSet)
         {
            ...
            spResultData = mitk::PointSetMappingHelper::map(inputSet, this->GetRegistration(), -1, false,
                           errorValue);
         }
