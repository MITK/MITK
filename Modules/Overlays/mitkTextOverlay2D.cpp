...
void mitk::TextOverlay2D::UpdateVtkOverlay2D(mitk::BaseRenderer *renderer)
{
  ...
  if (ls->IsGenerateDataRequired(renderer, this))
  {
    ...
    bool boldFont(false);
    GetBoolProperty( "font.bold", boldFont, renderer );
    ls->m_TextProp->SetBold( boldFont );

    bool italicFont(false);
    GetBoolProperty( "font.italic", italicFont, renderer );
    ls->m_TextProp->SetBold( italicFont );

    bool drawShadow;
    GetBoolProperty("drawShadow", drawShadow, renderer);
    ls->m_TextProp->SetShadow(drawShadow);

    ls->m_TextActor->SetInput(GetText(renderer).c_str());
