...
class mitkAnnotationPlacerTestSuite : public mitk::TestFixture
{
  CPPUNIT_TEST_SUITE(mitkAnnotationPlacerTestSuite);
  MITK_TEST(Render2DOverlays);
  MITK_TEST(Render3DOverlays);
...
public:
   ...
   * resolution.
   */
  mitkAnnotationPlacerTestSuite() : m_RenderingTestHelper(300, 300) {}
  /**
  ...
  void Render2DOverlays()
  {
    ...
    std::string refImagePath = GetTestDataFilePath(m_ReferenceImagePath + "AnnotationPlacer2DOverlays.png");
    ...
    mitk::TextOverlay2D::Pointer textOverlay2D = mitk::TextOverlay2D::New();

    textOverlay2D->SetText("TEST AnnotationPlacer2DOverlays");
    mitk::Point2D pos;
    pos[0] = 0;
    pos[1] = 0;
    textOverlay2D->SetPosition2D(pos);

    mitk::BaseRenderer *renderer = mitk::BaseRenderer::GetInstance(m_RenderingTestHelper.GetVtkRenderWindow());
    mitk::AnnotationPlacer::AddOverlay(textOverlay2D.GetPointer(), renderer);
  ...
  void Render3DOverlays()
  {
    ...
    std::string refImagePath = GetTestDataFilePath(m_ReferenceImagePath + "AnnotationPlacer3DOverlays.png");
    // reference screenshot for this test
    m_CommandlineArgs.push_back(refImagePath);
    // Convert vector of strings to argc/argv
    mitk::RenderingTestHelper::ArgcHelperClass arg(m_CommandlineArgs);
    ...
    textOverlay3D->SetText("TEST AnnotationPlacer3DOverlays");
    mitk::Point3D pos;
    pos[0] = 10;
    pos[1] = 10;
    pos[2] = 10;
    textOverlay3D->SetPosition3D(pos);

    mitk::BaseRenderer *renderer = mitk::BaseRenderer::GetInstance(m_RenderingTestHelper.GetVtkRenderWindow());
    mitk::AnnotationPlacer::AddOverlay(textOverlay3D.GetPointer(), renderer);
...
MITK_TEST_SUITE_REGISTRATION(mitkAnnotationPlacer)
