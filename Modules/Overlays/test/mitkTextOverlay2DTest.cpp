...
class mitkTextOverlay2DTestSuite : public mitk::TestFixture
{
  CPPUNIT_TEST_SUITE(mitkTextOverlay2DTestSuite);
...
public:
  /**
   * @brief mitkTextOverlay2DTestSuite Because the RenderingTestHelper does not have an
   * empty default constructor, we need this constructor to initialize the helper with a
   * resolution.
   */
  mitkTextOverlay2DTestSuite() : m_RenderingTestHelper(300, 300) {}
  /**
  ...
  void Render2DOverlays()
  {
    ...
    std::string refImagePath = GetTestDataFilePath(m_ReferenceImagePath + "TextOverlay2D.png");
    ...
    textOverlay2D->SetText("TEST TextOverlay2D2DOverlays");
    mitk::Point2D pos;
    pos[0] = 0;
    pos[1] = 0;
...
MITK_TEST_SUITE_REGISTRATION(mitkTextOverlay2D)
