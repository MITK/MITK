...
class mitkTextOverlay3DTestSuite : public mitk::TestFixture
{
  CPPUNIT_TEST_SUITE(mitkTextOverlay3DTestSuite);
...
public:
  /**
   * @brief mitkTextOverlay3DTestSuite Because the RenderingTestHelper does not have an
   * empty default constructor, we need this constructor to initialize the helper with a
   * resolution.
   */
  mitkTextOverlay3DTestSuite() : m_RenderingTestHelper(300, 300) {}
  /**
  ...
  void Render3DOverlays()
  {
    ...
    std::string refImagePath = GetTestDataFilePath(m_ReferenceImagePath + "TextOverlay3D.png");
    ...
    textOverlay3D->SetText("TESTAnnotationPlacer3DOverlays");
    mitk::Point3D pos;
    pos[0] = 10;
    pos[1] = 10;
    pos[2] = 10;
...
MITK_TEST_SUITE_REGISTRATION(mitkTextOverlay3D)
