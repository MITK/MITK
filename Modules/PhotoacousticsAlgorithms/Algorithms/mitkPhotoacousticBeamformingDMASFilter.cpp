...
void mitk::BeamformingDMASFilter::SetProgressHandle(std::function<void(int, std::string)> progressHandle)
{
...
void mitk::BeamformingDMASFilter::GenerateData()
{
  ...
  for (int i = 0; i < output->GetDimension(2); ++i) // seperate Slices should get Beamforming seperately applied
  {
    ...
    //threadpool... seems slower but I'll just keep it here for now
...
void mitk::BeamformingDMASFilter::DMASSphericalLine(double* input, double* output, double inputDim[2], double outputDim[2], const short& line, double* apodisation, const short& apodArraySize)
{
  double& inputS = inputDim[1];
  double& inputL = inputDim[0];
