 ...
 *
 *=========================================================================*/
#ifndef itkConnectedLabelImageFilter_hxx
#define itkConnectedLabelImageFilter_hxx

#include "itkConnectedLabelImageFilter.h"
...
void
ConnectedLabelImageFilter< TInputImage, TOutputImage, TMaskImage >
                       ...
                       ThreadIdType threadId)
{
  ...
  for ( inLineIt.GoToBegin();
    ...
    while ( !inLineIt.IsAtEndOfLine() )
      ...
      if ( m_MinValue <= PVal && PVal <= m_MaxValue )
        {
        ...
        ++inLineIt;
        while ( !inLineIt.IsAtEndOfLine()
                && m_MinValue <= inLineIt.Get() && inLineIt.Get() <= m_MaxValue )
          {
          ++length;
          ++inLineIt;
          }
...
::PrintSelf(std::ostream & os, Indent indent) const
{
  ...
  os << indent << "MinValue: "
     << static_cast< typename NumericTraits< OutputImagePixelType >::PrintType >( m_MinValue ) << std::endl;
  os << indent << "MaxValue: "
     << static_cast< typename NumericTraits< OutputImagePixelType >::PrintType >( m_MaxValue ) << std::endl;
}
} // end namespace itk
