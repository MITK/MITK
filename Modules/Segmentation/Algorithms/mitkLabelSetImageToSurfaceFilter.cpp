...
void mitk::LabelSetImageToSurfaceFilter::InternalProcessing( itk::Image<TPixel, VDimension>* input, mitk::Surface* surface )
{
...
//  thresholdFilter->ReleaseDataFlagOn();
  ...
  mitk::Geometry3D* newGeometry = m_ResultImage->GetSlicedGeometry();
  mitk::Point3D origin;
  vtk2itk(cropIndex, origin);
  this->GetInput()->GetGeometry()->IndexToWorld(origin, origin);
  newGeometry->SetOrigin(origin);

  vtkSmartPointer<vtkImageData> vtkimage = m_ResultImage->GetVtkImageData(0);
  ...
  cleanPolyDataFilter->SetInput(polydata);
