/*============================================================================

The Medical Imaging Interaction Toolkit (MITK)

Copyright (c) German Cancer Research Center (DKFZ)
All rights reserved.

Use of this source code is governed by a 3-clause BSD license that can be
found in the LICENSE file.

============================================================================*/

#include "mitkSegGroupRemoveOperation.h"

mitk::SegGroupRemoveOperation::SegGroupRemoveOperation(MultiLabelSegmentation* segmentation,
  const GroupIndexSetType& removalGroupIDs)
  : SegChangeOperationBase(segmentation, 1), m_GroupIDs(removalGroupIDs)
{
}

mitk::SegGroupRemoveOperation::GroupIndexSetType mitk::SegGroupRemoveOperation::GetGroupIDs() const
{
  return m_GroupIDs;
}

