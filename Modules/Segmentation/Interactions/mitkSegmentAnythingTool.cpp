...
{
  this->SetWorkingPlaneGeometry(const_cast<mitk::PlaneGeometry *>(
    interactionEvent->GetSender()->GetCurrentWorldPlaneGeometry())); // try to set without const cast
  MITK_INFO << "Set W PlaneG";
...
void mitk::SegmentAnythingTool::DoUpdatePreview(const Image* inputAtTimeStep, const Image* oldSegAtTimeStep, LabelSetImage* previewImage, TimeStepType timeStep)
{
  if (nullptr != oldSegAtTimeStep && nullptr != previewImage && m_PointSet.IsNotNull())
  {
    ...
    if (this->HasPicks())
    {
      ...
      inputImagePath =
        IOUtil::CreateTemporaryFile(tmpStream, TEMPLATE_FILENAME, inDir + IOUtil::GetDirectorySeparator());
      tmpStream.close();
      ...
      //auto pg = this->GetWorkingPlaneGeometry();
      //this->run_generate_embeddings(spExec, inputImagePath, outDir, this->GetGpuId());

      outputImagePath = "C:\\DKFZ\\SAM_work\\test_seg_3d.nii.gz";
      Image::Pointer outputImage = IOUtil::Load<Image>(outputImagePath);
      previewImage->InitializeByLabeledImage(outputImage);
      //const_cast<mitk::PlaneGeometry *>(this->GetWorkingPlaneGeometry())->ChangeImageGeometryConsideringOriginOffset(true);
      previewImage->SetGeometry(const_cast<mitk::PlaneGeometry *> (this->GetWorkingPlaneGeometry()));
    }
  }
}

void mitk::SegmentAnythingTool::run_generate_embeddings(ProcessExecutor::Pointer spExec,
