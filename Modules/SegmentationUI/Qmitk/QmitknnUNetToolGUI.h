...
private:
  ...
  /**
   * @brief Returns number of Nvidia GPUS available in the machine.
   * 
   * The method essentially executes (Linux) `nvidia-smi -L | wc -l` or
  ...
  /**
   * @brief Stores row count of the "advancedSettingsLayout" layout element. This value helps dynamically add ctk-path-line-edit
   * UI elements at the right place.
   * Forced to initialize in the InitializeUI method since there is no guarantee of retrieving exact row count anywhere else.
   * 
   */
  int m_UI_ROWS;
};
