...
/**
* \brief Inspector widget providing a tree view on labels and groups of a MultiLabelSegmentation instance.
*
* Also allows manipulation operations on the labels/groups according to the UI/selection state.
*/
...
public:
  /** \brief Constructs the multi-label inspector widget. */
  QmitkMultiLabelInspector(QWidget* parent = nullptr);

  ~QmitkMultiLabelInspector();

  /** \brief Returns whether multi-selection mode is enabled. */
  bool GetMultiSelectionMode() const;

  /** \brief Returns whether visibility modification is allowed. */
  bool GetAllowVisibilityModification() const;
  /** \brief Returns whether lock modification is allowed. */
  bool GetAllowLockModification() const;
  /** \brief Returns whether label modification (add/remove/rename) is allowed. */
