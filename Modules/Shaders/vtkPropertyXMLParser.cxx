/*=========================================================================
  ...
  Module:    vtkPropertyXMLParser.cxx
...
vtkCxxSetObjectMacro(vtkPropertyXMLParser, ShaderProgram, vtkShaderProgram2);
//----------------------------------------------------------------------------
// Return NULL if no override is supplied.
vtkAbstractObjectFactoryNewMacro(vtkPropertyXMLParser)
//----------------------------------------------------------------------------
...
vtkPropertyXMLParser::vtkPropertyXMLParser()
{
...
void vtkPropertyXMLParser::DeepCopy(vtkPropertyXMLParser *p)
{
  if (p != NULL)
    {
...
//----------------------------------------------------------------------------
void vtkPropertyXMLParser::SetColor(double r, double g, double b)
{
...
//----------------------------------------------------------------------------
void vtkPropertyXMLParser::SetColor(double a[3])
{
  this->SetColor(a[0], a[1], a[2]);
}

//----------------------------------------------------------------------------
void vtkPropertyXMLParser::ComputeCompositeColor(double result[3],
...
double *vtkPropertyXMLParser::GetColor()
{
  vtkPropertyXMLParser::ComputeCompositeColor(this->Color,
...
// provided.
void vtkPropertyXMLParser::GetColor(double rgb[3])
{
  this->GetColor();
  rgb[0] = this->Color[0];
  rgb[1] = this->Color[1];
...
//----------------------------------------------------------------------------
void vtkPropertyXMLParser::GetColor(double &r, double &g, double &b)
{
  this->GetColor();
  r = this->Color[0];
  g = this->Color[1];
...
//----------------------------------------------------------------------------
void vtkPropertyXMLParser::SetTexture(const char* name, vtkTexture* tex)
{
...
//----------------------------------------------------------------------------
vtkTexture* vtkPropertyXMLParser::GetTexture(const char* name)
{
  vtkPropertyInternals::MapOfTextureNames::iterator iter =
    this->Internals->TextureNames.find(vtkStdString(name));
  if (iter == this->Internals->TextureNames.end())
...
//----------------------------------------------------------------------------
void vtkPropertyXMLParser::SetTexture(int unit, vtkTexture* tex)
{
  vtkPropertyInternals::MapOfTextures::iterator iter =
...
//----------------------------------------------------------------------------
vtkTexture* vtkPropertyXMLParser::GetTexture(int unit)
{
  vtkPropertyInternals::MapOfTextures::iterator iter =
    this->Internals->Textures.find(unit);
  if (iter != this->Internals->Textures.end())
...
//----------------------------------------------------------------------------
int vtkPropertyXMLParser::GetNumberOfTextures()
{
  return static_cast<int>(this->Internals->Textures.size());
}

//----------------------------------------------------------------------------
void vtkPropertyXMLParser::RemoveTexture(const char* name)
{
  vtkPropertyInternals::MapOfTextureNames::iterator iter =
    this->Internals->TextureNames.find(vtkStdString(name));
  if (iter != this->Internals->TextureNames.end())
...
//----------------------------------------------------------------------------
void vtkPropertyXMLParser::RemoveTexture(int unit)
{
  vtkPropertyInternals::MapOfTextures::iterator iter =
    this->Internals->Textures.find(unit);
  if (iter != this->Internals->Textures.end())
...
//----------------------------------------------------------------------------
void vtkPropertyXMLParser::RemoveAllTextures()
{
...
//----------------------------------------------------------------------------
vtkTexture* vtkPropertyXMLParser::GetTextureAtIndex(int index)
{
  vtkPropertyInternals::MapOfTextures::iterator iter =
...
//----------------------------------------------------------------------------
int vtkPropertyXMLParser::GetTextureUnitAtIndex(int index)
{
  vtkPropertyInternals::MapOfTextures::iterator iter =
    this->Internals->Textures.begin();
  for (int id = 0; iter != this->Internals->Textures.end(); ++iter, ++id)
...
//----------------------------------------------------------------------------
int vtkPropertyXMLParser::GetTextureUnit(const char* name)
{
  vtkPropertyInternals::MapOfTextureNames::iterator iter =
...
//----------------------------------------------------------------------------
void vtkPropertyXMLParser::LoadMaterial(const char* name)
{
  this->SetMaterialName(0);
...
//----------------------------------------------------------------------------
void vtkPropertyXMLParser::LoadMaterialFromString(const char* materialxml)
{
  this->SetMaterialName(0);
  if (!materialxml)
    {
...
void vtkPropertyXMLParser::ReadFrameworkMaterial()
{
  // empty. See vtkOpenGLProperty.
}

//----------------------------------------------------------------------------
void vtkPropertyXMLParser::LoadMaterial(vtkXMLMaterial* material)
{
  this->SetMaterialName(0);
...
//        shaders->Delete();
      ...
      {
/*      vtkShaderProgram2* shader = vtkShaderProgram2::CreateShaderProgram(lang); TODO create shader???
      if (shader)
        {
      ...
      // Some materials may have no shaders and only set ivars for vtkPropertyXMLParser.
...
//----------------------------------------------------------------------------
void vtkPropertyXMLParser::LoadProperty()
{
...
//----------------------------------------------------------------------------
void vtkPropertyXMLParser::LoadTextures()
{
...
//----------------------------------------------------------------------------
void vtkPropertyXMLParser::LoadMember(vtkXMLDataElement* elem)
{
...
//----------------------------------------------------------------------------
void vtkPropertyXMLParser::LoadTexture(vtkXMLDataElement* elem )
{
  const char* name = elem->GetAttribute("name");
  if (!name)
    {
...
//----------------------------------------------------------------------------
void vtkPropertyXMLParser::LoadPerlineNoise(vtkXMLDataElement* )
{
  vtkWarningMacro("Perlin Noise support not complete yet!");
}

//----------------------------------------------------------------------------
void vtkPropertyXMLParser::Render(vtkActor* actor, vtkRenderer* renderer)
{
...
//----------------------------------------------------------------------------
void vtkPropertyXMLParser::PostRender(vtkActor* actor, vtkRenderer* renderer)
{
  if (renderer->GetSelector())
    {
    // nothing to do when rendering for hardware selection.
...
//----------------------------------------------------------------------------
void vtkPropertyXMLParser::AddShaderVariable(const char* name, int numVars, int* x)
{
...
//----------------------------------------------------------------------------
void vtkPropertyXMLParser::AddShaderVariable(const char* name, int numVars, float* x)
{
  if (!this->ShaderProgram)
    {
    return;
...
//----------------------------------------------------------------------------
void vtkPropertyXMLParser::AddShaderVariable(const char* name, int numVars, double* x)
{
  if (!this->ShaderProgram)
    {
    return;
...
//-----------------------------------------------------------------------------
void vtkPropertyXMLParser::ReleaseGraphicsResources(vtkWindow *win)
{
  if (this->ShaderProgram)
    {
...
//----------------------------------------------------------------------------
void vtkPropertyXMLParser::PrintSelf(ostream& os, vtkIndent indent)
{
