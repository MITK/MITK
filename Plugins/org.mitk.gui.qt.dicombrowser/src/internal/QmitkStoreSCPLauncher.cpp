...
using Self = QmitkStoreSCPLauncher;
...
: m_StoreSCP(new QProcess)
{
  ...
  connect(m_StoreSCP, &QProcess::errorOccurred, this, &Self::OnProcessError);
  connect(m_StoreSCP, &QProcess::stateChanged, this, &Self::OnStateChanged);
  connect(m_StoreSCP, &QProcess::readyReadStandardOutput, this, &Self::OnReadyProcessOutput);

  this->SetArgumentList(builder);
}

QmitkStoreSCPLauncher::~QmitkStoreSCPLauncher()
{
  disconnect(m_StoreSCP, &QProcess::errorOccurred, this, &Self::OnProcessError);
  disconnect(m_StoreSCP, &QProcess::stateChanged, this, &Self::OnStateChanged);
  disconnect(m_StoreSCP, &QProcess::readyReadStandardOutput, this, &Self::OnReadyProcessOutput);
...
void QmitkStoreSCPLauncher::SetArgumentList(QmitkStoreSCPLauncherBuilder* builder)
{
  m_ArgumentList
    << *builder->GetPort()
    << QString("-aet") << *builder->GetAETitle()
    << *builder->GetTransferSyntax()
    << *builder->GetOtherNetworkOptions()
    << *builder->GetMode()
    << QString("-od") << *builder->GetOutputDirectory();
}
