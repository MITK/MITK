...
void QmitkPreprocessingView::OnSelectionChanged( std::vector<mitk::DataNode*> nodes )
{
  ...
  if (foundDwiVolume)
  {
    ...
    BValueMap originalBValueMap = m_DiffusionImage->GetB_ValueMap();
    QCheckBox* checkBox;
    QSpinBox* spinBox;
    for(it = originalBValueMap.begin() ;it != originalBValueMap.end(); it++)
    {
      if(it->first != 0 && originalBValueMap.size() > 1){
