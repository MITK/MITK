...
void QmitkExtFileSaveProjectAction::Run()
{
  try
  {
    ...
    if (!dsService)
    {
      QString msg = "IDataStorageService service not available. Unable to save project.";
      MITK_WARN << msg.toStdString();
      QMessageBox::warning(QApplication::activeWindow(), "Unable to save project", msg);
      return;
    }

    // Get the active data storage info (or the default one, if none is active)
