...
void USNavigationMarkerPlacement::OnFinishExperiment()
{
  ...
  writer.Write(QString(m_ExperimentResultsSubDirectory + QDir::separator() + "navigation-data.xml").toStdString().c_str(), m_NavigationDataRecorder->GetNavigationDataSet());

  mitk::NavigationDataSetWriterCSV writerCSV;
  writerCSV.Write(QString(m_ExperimentResultsSubDirectory + QDir::separator() + "navigation-data.csv").toStdString().c_str(), m_NavigationDataRecorder->GetNavigationDataSet());
