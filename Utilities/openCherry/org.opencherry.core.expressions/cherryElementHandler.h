...
public:
   ...
   */
  virtual Expression::Pointer Create(ExpressionConverter* converter, IConfigurationElement* config) = 0;
...
protected:
   ...
   * @throws CoreException if the conversion failed
   */
  virtual void ProcessChildren(ExpressionConverter* converter, IConfigurationElement* element, CompositeExpression* expression);
   ...
   * @since 3.3
   */
  virtual void ProcessChildren(ExpressionConverter* converter, Poco::XML::Element* element, CompositeExpression* expression);
};

} // namespace cherry

#endif /*CHERRYELEMENTHANDLER_H_*/
