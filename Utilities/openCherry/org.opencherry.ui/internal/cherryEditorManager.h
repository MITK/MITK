...
public:
   ...
   */
private:
  void FindEditors(std::vector<IEditorReference::Pointer>& editorList,
      IEditorInput::Pointer input, const std::string& editorId, int matchFlags,
...
public:
  IEditorReference::Pointer OpenEditor(const std::string& editorId,
      IEditorInput::Pointer input, bool setVisible, IMemento* editorState);
...
protected:
  IEditorPart::Pointer CreatePart(EditorDescriptor::Pointer desc);
...
protected:
   ...
   */
public:
  /*IStatus*/void RestoreState(IMemento* memento);
...
public:
  static bool SaveAll(const std::vector<IWorkbenchPart::Pointer>& dirtyParts,
      bool confirm, bool closing, bool addNonPartSources,
   ...
   */
public:
  bool SavePart(/*ISaveablePart::Pointer saveable,*/ SmartPointer<IWorkbenchPart> part,
      bool confirm);
   ...
   */
public:
  /*IStatus*/void SaveState(const IMemento* memento);
...
private:
   ...
   */
private:
  void RestoreEditorState(IMemento* editorMem,
      std::vector<IEditorReference::Pointer>& visibleEditors,
      std::vector<IEditorReference::Pointer>& activeEditor);

  // for dynamic UI
protected:
  void SaveEditorState(IMemento* mem, IEditorReference::Pointer ed/*, MultiStatus res*/);

  // for dynamic UI
public:
  IMemento* GetMemento(IEditorReference::Pointer e);
