...
namespace cherry {
 ...
 IExtensionPoint::Point WorkbenchPage::GetPerspectiveExtensionPoint() {
 ...
 WorkbenchPage::WorkbenchPage(WorkbenchWindow::Pointer w, const std::string& layoutID, IAdaptable input)
            {
        super();
        if (layoutID == null) {
   ...
   WorkbenchPage::WorkbenchPage(WorkbenchWindow::Pointer w, IAdaptable input)
    ...
    void WorkbenchPage::ActivatePart(const IWorkbenchPart::ConstPointer part) {
    ...
    void WorkbenchPage::AddSelectionListener(const std::string& partId, ISelectionListener listener) {
    ...
    ILayoutContainer WorkbenchPage::GetContainer(IWorkbenchPartReference::Pointer part) {
        PartPane pane = getPane(part);
        if (pane == null) {
            return null;
        }
    ...
    PartPane WorkbenchPage::GetPane(IWorkbenchPart::Pointer part) {
        if (part == null) {
            return null;
        }
    ...
    PartPane WorkbenchPage::GetPane(IWorkbenchPartReference::Pointer part) {
        if (part == null) {
            return null;
        }
    ...
    bool WorkbenchPage::InternalBringToTop(IWorkbenchPartReference::Pointer part) {
     ...
     IViewPart::Pointer WorkbenchPage::BusyShowView(const std::string& viewID, const std::string& secondaryID, int mode)
            {
        Perspective persp = getActivePerspective();
        if (persp == null) {
      return null;
     ...
     bool WorkbenchPage::CloseEditors(std::vector<IEditorReference::Pointer> refArray, bool save) {
   ...
   void WorkbenchPage::partAdded(WorkbenchPartReference ref) {
        activationList.add(ref);
        partList.addPart(ref);
        updateActivePart();
    }
    
   void WorkbenchPage::partRemoved(WorkbenchPartReference ref) {
...
//                                Status.OK, "WorkbenchPage leaked a refcount for view " + ref.getId(), null));  //$NON-NLS-1$//$NON-NLS-2$
                    ...
                    public void WorkbenchPage::handleException(Throwable e) {
                    }
                });
            }
        }
