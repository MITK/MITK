...
{
  //TODO DetachedWindow shell creation
...
//     ->GetDetachedWindowPool()->AllocateShell(shellListener);
  ...
  if (bounds.IsEmpty())
  {
    ...
    Point center(windowRect.x + windowRect.width / 2, windowRect.y - windowRect.height / 2);
...
//    bounds.y = dispBounds.height - bounds.height;
  ...
  this->CreateContents(windowShell);
...
{
  this->UpdateTitle();
  //TODO DetachedWindow resize, activate listeners
...
void* DetachedWindow::CreateContents(void* parent)
{
  ...
  for (std::list<PartPane::Pointer>::iterator itr = detachedChildren.begin();
         itr != detachedChildren.end(); ++itr)
  {
...
bool DetachedWindow::HandleClose()
{
  ...
  if (hideViewsOnClose)
  {
    ...
    for (std::list<PartPane::Pointer>::iterator itr = views.begin();
           itr != views.end(); ++itr)
    {
...
//  }
  ...
  if (windowShell != 0)
  {
    //TODO DetachedWindow resize, activate listener
...
bool DetachedWindow::HandleSaves(std::list<PartPane::Pointer> views)
{
  ...
  for (std::list<PartPane::Pointer>::iterator iterator = views.begin();
       iterator != views.end(); ++iterator)
  {
  ...
  if (dirtyViews.size() > 0)
  {
    ...
    bool success = EditorManager::SaveAll(dirtyViews, true, true, false,
        window);
    if (!success)
    {
