...
void EditorRegistryReader::AddEditors(EditorRegistry* registry) {
 ...
 bool EditorRegistryReader::ReadElement(IConfigurationElement::Pointer element) {
    ...
    std::vector<std::string> contentTypeVector;
        ...
        if (element->GetAttribute(WorkbenchRegistryConstants::ATT_EXTENSIONS, extensionsString)) {
            Poco::StringTokenizer tokenizer(extensionsString, ",", Poco::StringTokenizer::TOK_IGNORE_EMPTY | Poco::StringTokenizer::TOK_TRIM);//$NON-NLS-1$
        ...
        if (element->GetAttribute(WorkbenchRegistryConstants::ATT_FILENAMES, filenamesString)) {
            Poco::StringTokenizer tokenizer(filenamesString, ",", Poco::StringTokenizer::TOK_IGNORE_EMPTY | Poco::StringTokenizer::TOK_TRIM);//$NON-NLS-1$
            Poco::StringTokenizer::Iterator token = tokenizer.begin();
            while (token != tokenizer.end()) {
    ...
    IConfigurationElement::vector bindings = element->GetChildren(WorkbenchRegistryConstants::TAG_CONTENT_TYPE_BINDING);
    for (unsigned int i = 0; i < bindings.size(); ++i) {
      std::string contentTypeId;
      if (!bindings[i]->GetAttribute(WorkbenchRegistryConstants::ATT_CONTENT_TYPE_ID, contentTypeId)) {
        continue;
      }
      contentTypeVector.push_back(contentTypeId);
        ...
        if (element->GetAttribute(WorkbenchRegistryConstants::ATT_DEFAULT, def)) {
          ...
          if (def == "1") defaultEditor = true;
          else {
            Poco::toUpperInPlace(def);
            if (def == "TRUE") defaultEditor = true;
          }
    }
