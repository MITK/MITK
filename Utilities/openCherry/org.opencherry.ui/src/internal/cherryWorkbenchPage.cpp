...
WorkbenchPage::ActivationOrderPred::ActivationOrderPred(PartList* pl)
 : partList(pl)
{
...
bool WorkbenchPage::
ActivationOrderPred::operator()(const IViewReference::Pointer o1, const IViewReference::Pointer o2) const
{
  int pos1 = partList->IndexOf(o1);
  int pos2 = partList->IndexOf(o2);
  return pos1 < pos2;
}
    ...
    IPerspectiveDescriptor::Pointer perspective, int newLoc)
{
  ...
  Perspective::Pointer movedPerspective = 0;
...
bool WorkbenchPage::PerspectiveList::Remove(Perspective::Pointer perspective)
{
  ...
  {
    this->UpdateActionSets(active, 0);
    active = 0;
  }
...
bool WorkbenchPage::Close()
{
  ...
  //    public void WorkbenchPage::run()
  //      {
  ret = window->ClosePage(this, true);
  //      }
  //    });
  return ret;
}
...
void WorkbenchPage::UpdateActivePart()
{
  ...
  if (!window->IsClosing())
  {
    ...
    {
      newActiveEditor = partList->GetActiveEditorReference();
      newActivePart = newActiveEditor.Cast<IWorkbenchPartReference>();
      if (newActivePart == 0)
      {
        // Only activate a non-editor if there's no editors left
        newActivePart = partList->GetActivePartReference();
      }
    }
    else
    {
...
void WorkbenchPage::MakeActiveEditor(IEditorReference::Pointer ref)
{
  ...
  IEditorPart::Pointer part = (ref == 0) ? 0 : ref->GetEditor(true);
    ...
    const std::list<IEditorReference::Pointer>& refArray, bool save)
{
  ...
  for (unsigned int i = 0; i < editorRefs.size(); i++)
  {
    ...
    window->FirePerspectiveChanged(this, this->GetPerspective(), ref,
        CHANGE_EDITOR_CLOSE);
  ...
  window->FirePerspectiveChanged(this, this->GetPerspective(),
      CHANGE_EDITOR_CLOSE);
    ...
    PerspectiveDescriptor::Pointer desc, bool notify)
{
  ...
  try
  {
    ...
    Perspective::Pointer persp = new Perspective(desc, this);
    perspList.Add(persp);
    if (notify)
    {
...
void WorkbenchPage::HideView(IViewReference::Pointer ref)
{
  ...
  SaveablesList::Pointer saveablesList = 0;
  SaveablesList::PostCloseInfo::Pointer postCloseInfo;
  if (refCount == 1)
  {
  ...
  window->FirePerspectiveChanged(this, persp->GetDesc(), ref, CHANGE_VIEW_HIDE);
  ...
  window->FirePerspectiveChanged(this, this->GetPerspective(), CHANGE_VIEW_HIDE);
}
    ...
    IViewPart::Pointer part)
{
  ...
  if (container.Cast<PartStack> () != 0)
  {
    ...
    std::sort(list.begin(), list.end(), ActivationOrderPred(partList));
