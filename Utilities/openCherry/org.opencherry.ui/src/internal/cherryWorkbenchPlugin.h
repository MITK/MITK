...
private:
  ...
  //static const std::string UI_BUNDLE_ACTIVATOR = "org.eclipse.ui.internal.UIPlugin"; //$NON-NLS-1$
...
//            const std::string& targetID, const std::string& base) {
//        IExtensionPoint* extensionPoint = Platform::GetExtensionPointService()
...
//        try {
//            return targetElement->CreateExecutableExtension<C>(base, "class"); //$NON-NLS-1$
//        } catch (CoreException e) {
//            // log it since we cannot safely display a dialog.
...
//    ISharedImages getSharedImages();
     ...
     * @param t
     */
    static void Log(const PlatformException& exc);
   ...
   */
    static void Log(const std::string& message, const PlatformException& t);
     ...
     *            The throwable from where the problem actually occurred.
     */
    static void Log(const std::string& clazz, const std::string& methodName, const PlatformException& t);
