...
public:
  ...
  void InitPython()
  {
    ...
    PyRun_SimpleString(pySysPath.c_str());
    PyRun_SimpleString("print(sys.path)");
    PyRun_SimpleString("import pyMITK");
    PyRun_SimpleString("print(pyMITK.SayHi(), flush=True)");
  }
  ...
  void TestLoadImage()
  {  
    MITK_INFO <<  "LoadImage: "<< m_ImagePath;
    PyGILState_Ensure();
	...
	MITK_INFO << "C++ received image has dimension: " << imagePtrReceived->GetDimension();
    ...
    {   Py_ssize_t n_dims = PyTuple_Size(pyTuple);
		for(Py_ssize_t i = 0; i < n_dims; i++) 
        {
            ...
            CPPUNIT_ASSERT_EQUAL(
                mitkImage->GetDimension(n_dims-i-1), static_cast<unsigned int>(PyLong_AsLong(value)));
        }
    }
  }
